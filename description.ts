import { Description, BillingPeriod, InboundShipment, ProductQuantity, Product, Size } from "./types";
import { parseISO, isAfter } from "date-fns";

function parseBillingPeriod(billingPeriod?: {from_date?: string, to_date?: string}): BillingPeriod {
...
function parseInboundShipment(shipment: ShipmentData): InboundShipment {
  if (!shipment.id || !Array.isArray(shipment.lines) || !shipment.offload_complete_time) {
    throw new Error('ERROR_DESCRIPTION')
  }
...
function parseProduct(product: DataProduct): Product {
  if(!product.id || !(product.size in Size) || !product.create_time) {
...
export function dataToDescription(data: any): Description {
  ...
  const inboundShipments: InboundShipment[] = parseInboundShipments(data.inbound_shipments);
